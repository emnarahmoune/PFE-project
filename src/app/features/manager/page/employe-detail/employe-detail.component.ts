...
export class EmployeDetailComponent implements OnInit {
  ...
  loadEmploye(id: number): void {
    this.employeService.getById(id).subscribe({
      next: (res: any) => {
        this.loading = false;
        if (res.success) this.employe = res.data;
  ...
  loadManagers(): void {
    this.managerService.getAllManagers().subscribe({
      next: (response) => {
        if (response.success) this.managersList = response.data;
      },
      error: () => this.managersList = []
    });
  }
    ...
    forkJoin({
      ...
      next: (res: any) => {
        ...
        if (Array.isArray(absData) && absData.length > 0) {
          const monAbs = absData.find((a: any) => a.employeId === employeId);
